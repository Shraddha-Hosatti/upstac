...
set -x
jq 'del(.channel_group.groups.Application.groups.insuranceMSP)' config.json > modified_config.json
set +x
...
echo
echo "========= Submitting transaction from a different peer (peer0.hospitalB) which also signs it ========= "
echo
setGlobals 0 "hospitalB"
set -x
