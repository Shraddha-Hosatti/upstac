...
function clearContainers() {
  ...
  CONTAINER_IDS=$(docker ps -a | awk '($2 ~ /dev-peer.*.common.*/) {print $1}')
...
function networkDown() {
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    docker run -v "$PWD":/tmp/upstacChannel --rm hyperledger/fabric-tools:"$IMAGETAG" rm -Rf /tmp/upstacChannel/ledgers-backup
...
function replacePrivateKey() {
  ...
  CURRENT_DIR=$PWD
  cd crypto-config/peerOrganizations/hospitalA.upstac.com/ca/ || exit
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR" || exit
  sed $OPTS "s/HOSPITALA_CA_PRIVATE_KEY/${PRIV_KEY}/g" docker-compose-e2e.yml
  cd crypto-config/peerOrganizations/hospitalB.upstac.com/ca/ || exit
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR" || exit
  sed $OPTS "s/HOSPITALB_CA_PRIVATE_KEY/${PRIV_KEY}/g" docker-compose-e2e.yml
  cd crypto-config/peerOrganizations/government.upstac.com/ca/ || exit
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR" || exit
  sed $OPTS "s/GOVERNMENT_CA_PRIVATE_KEY/${PRIV_KEY}/g" docker-compose-e2e.yml
...
# Generates Org certs using cryptogen tool
function generateCertsInsurance() {
  which cryptogen
  if [ "$?" -ne 0 ]; then
    echo "cryptogen tool not found. exiting"
    exit 1
...
function copyOrdererToInsurance() {
  echo
  echo "##########################################################"
  echo "##### Copy Orderer certificates #########"
  echo "##########################################################"
...
function generateChannelArtifacts() {
  ...
  echo "#################################################################"
  set -x
  configtxgen -profile common -outputCreateChannelTx ./channel-artifacts/channel.tx -channelID "$CHANNEL_NAME"
  res=$?
  set +x
  if [ $res -ne 0 ]; then
  ...
  echo
  echo "#################################################################"
  echo "#######    Generating anchor peer update for hospitalAMSP   ##########"
  echo "#################################################################"
  set -x
  configtxgen -profile common -outputAnchorPeersUpdate ./channel-artifacts/hospitalAMSPanchors.tx -channelID "$CHANNEL_NAME" -asOrg hospitalAMSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
    echo "Failed to generate anchor peer update for hospitalA..."
    exit 1
  fi

  echo
  echo "#################################################################"
  echo "#######    Generating anchor peer update for hospitalBMSP   ##########"
  echo "#################################################################"
  set -x
  configtxgen -profile common -outputAnchorPeersUpdate ./channel-artifacts/hospitalBMSPanchors.tx -channelID "$CHANNEL_NAME" -asOrg hospitalBMSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
    echo "Failed to generate anchor peer update for hospitalB..."
    exit 1
  fi
  echo
  ...
  echo "#################################################################"
  set -x
  configtxgen -profile common -outputAnchorPeersUpdate ./channel-artifacts/governmentMSPanchors.tx -channelID "$CHANNEL_NAME" -asOrg governmentMSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
    echo "Failed to generate anchor peer update for government..."
    exit 1
  fi
  echo
...
function updateCommonWithInsurance(){
  checkPrereqs
  VERSION_NO=5.0
  docker exec cli scripts/updateChaincodeWithInsuranceStep1.sh "$CHANNEL_NAME" "$CLI_DELAY" "$LANGUAGE" "$VERSION_NO" "$TYPE"
  docker exec insurancecli scripts/updateChaincodeWithInsuranceStep2.sh "$CHANNEL_NAME" "$CLI_DELAY" "$LANGUAGE" "$VERSION_NO" "$TYPE"
  docker exec cli scripts/updateChaincodeWithInsuranceStep3.sh "$CHANNEL_NAME" "$CLI_DELAY" "$LANGUAGE" "$VERSION_NO" "$TYPE"
}
...
elif [ "$MODE" == "generateCertsInsurance" ]; then
  EXPMODE="Generate Insurance Certs"
elif [ "$MODE" == "copyOrdererToInsurance" ]; then
  EXPMODE="Copy Orderer"
elif [ "$MODE" == "updateCommonWithInsurance" ]; then
  EXPMODE="Updating chaincode on all 4 orgs"
else
  printHelp
  exit 1
fi
...
if [ "${MODE}" == "up" ]; then
  ...
  generateCertsInsurance  
elif [ "${MODE}" == "copyOrdererToInsurance" ]; then
  copyOrdererToInsurance
elif [ "${MODE}" == "upInsurance" ]; then
  upInsurance
elif [ "${MODE}" == "updateCommonWithInsurance" ]; then
  updateCommonWithInsurance
else
  printHelp
  exit 1
fi
